...
$images | foreach {
    ...
    if ($osType -eq 'Windows')
    {
        if ($enableWinRM)
        {
            show-progress "WinRM cert URL: $certURL"
            $global:vm = $vm
            $vm = Set-AzureRmVMOperatingSystem -VM $vm -Windows -ComputerName $vmName -Credential $credential -ProvisionVMAgent -WinRMHttp -WinRMHttps -WinRMCertificateUrl $certURL
            $vm = Add-AzureRmVMSecret -VM $vm -SourceVaultId $vaultId -CertificateStore $certStore -CertificateUrl $certURL
        }
        else
        {
            $vm = Set-AzureRmVMOperatingSystem -VM $vm -Windows -ComputerName $vmName -Credential $credential -ProvisionVMAgent
            if ($noga)
            {
                $vm.OSProfile.WindowsConfiguration.ProvisionVMAgent = $false
            }
